function [k, theta, condition_number, time_taken, ls, md] = ...
    ...
    a_num, c_num, L, OptTheta, OptL)

%a_num = 1, c_num=4, c_tilda_num=8, alpha=1, gamma=gamma_tilda=1/6, sigma=2
%InitTheta = OptTheta* 0.5
...
while budget_cntr <= budget-5
    ...
    % Create Hessian
    if (k>0)
        wkhk=((Del_tilda_Y-Del_Y)/((2*((k+1)*c_k))*c_tilda_k));
        kp1bk=(k+1)/k;
        wkhk_by_OneMwk=wkhk*kp1bk;
        middleton=(Bbar*del_tilda_k);
        deno=1+wkhk_by_OneMwk*del_k'*middleton;
        Bbar = Bbar*kp1bk - ((wkhk_by_OneMwk/deno)*kp1bk*...
            middleton)*(del_k'*Bbar);
    end
    % Update Theta
    theta_delta=(a(k)/c_k)*(Del_Y/2)*(Bbar*del_k);
