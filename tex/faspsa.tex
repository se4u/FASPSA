...
\section{Problem Formulation in Enhancement for Adaptive SPSA Algorithms}
In the following discussion, use the following notation. For a vector $ \bm{x}\in \mathbbm{R}^p $, denote $ [1/x_1, ..., 1/x_p]^T $ as $ \bm{x}^{-1} $, and $ [1/x_1, ..., 1/x_p] $ as $ \bm{x}^{-T} $.Obviously, $ \bm{x}^{-T}\bm{x}=p $.
...
Typically, we only have access to the noisy function measurement of $ L(\bm{\uptheta}) $, say $ y(\bm{\uptheta})=L(\bm{\uptheta})+\upvarepsilon(\bm{\uptheta}) $, or noisy measurement of gradient evaluation. In this paper we only consider the former case.

Adaptive simultaneous perturbation (ASP) algorithm incorporated the feedback and weighting mechanism in \cite{Spall2009} is as following:
...
\end{align*}
$ a_k $, $ c_k $ and $ \tilde{c}_k $ are all non-negative scalar gain coefficients, $ \bDelta_k $ and $ \tDelta_k $ are stochastic perturbation (vector) sequence, and the pre-conditioning function $ \bm{f}_k $ is to maintain the symmetry and positive-definiteness of $ \ooH_k $.

Through out our discussion, all components in the perturbation sequences (both $ c_k $ and $ \tilde{c}_k $) are \textit{Bernoulli} distributed.
...
Now we first consider the update of the inverse of $ \oH_k $, then later do the  conditioning on $ \oH_k $ to obtain $ \ooH_k $. Eq. (3.8) in Spall \cite{Spall2009} provides a symmetrized $ \hPsi_k $.  Below is the pre-symmetrized from of $ \hPsi_k $:
\begin{equation}
	\hPhi_k=\tD_k^T\oH_{k-1}\bD_k+\tD_k^T\oH_{k-1}+\oH_{k-1}\bD_k
\end{equation}
where $ \bD_k=\bDelta_k\bDelta_k^{-T}-\bI_p, \tD_k=\tDelta_k\tDelta_k^{-T}-\bI_p $. Obviously,
\begin{equation} \label{eq:PsiHat}
\hPsi_k =(\hPhi_k+\hPhi_k^T)/2
\end{equation}


Bar-Itzhack \cite{Bar-Itzhack1998} shows that for any matrix $ \bm{P}\in\mathbb{R}^{p\times p} $, the closest symmetric matrix to $ \bm{P} $, in Frobenius (Euclidean) norm, is $ (\bm{P}+\bm{P}^T)/2 $. So it's reasonable how equation~\ref{eq:Hhat} and equation~\ref{eq:PsiHat} proceed the symmetrization.
...
Recall Woodbury matrix identity,
\begin{equation*}
...
We can obtain a sequential recursion of the $ \oH_k^{-1} $ as following:
\begin{equation}
	\begin{cases}
	\bm{B}_k^{-1}&=\oH_{k-1}^{-1}-\oH_{k-1}^{-1}\tDelta_k^{-1}(b_k^{-1}+\bDelta_k^{-T}\oH_{k-1}^{-1}\tDelta_k^{-1})^{-1}\bDelta_k^{-T}\oH_{k-1}^{-1}\\% \nonumber
    \oH_k^{-1}&=\bm{B}_k^{-1}-\bm{B}_k^{-1}\bDelta_k^{-1}(b_k^{-1}+\tDelta_k^{-T}\bm{B}_k^{-1}\bDelta_k^{-1})^{-1}\tDelta_k^{-T}\bm{B}_k^{-1}
\end{cases}
\end{equation}
...
Use same test function in \cite{Spall2009}, the quartic function, we do a comparison study.
