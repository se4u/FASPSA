...
\section{Conclusion}
As discussed in section \ref{2SPSA} to \ref{Enhanced 2SG}, the recursion of the Hessian inverse can be achieved via a rank-2 update, which breaks down to a sequential rank-1 update. Basically, the Hessian update (with or without feedback term) can be written as:
...
&~~~\cdot(\frac{1}{b_k}+\bm{u}_k^{T}\oH_{k-1}^{-1}\bm{v}_k)^{-1}\bm{u}_k^{T}\oH_{k-1}^{-1}
\end{dcases}
\end{equation}
where
\begin{equation}
