...
\section{Efficient Update for Hessian Inverse} \label{Efficient Update for Hessian Inverse}
...
Page 12 in Farebrother \cite{Farebrother1988} shows that the total number $2p^3/3+3p^2/2-7p/6$ operations is required by the Gaussian-Jordan elimination. When $p>7$ in applying basic \textit{adaptive} algorithms, or when $p>15$ in applying \textit{enhanced} algorithms, the corresponding number
of operations is fewer than $2p^3/3+3p^2/2-7p/6$ when computing the term $\ooH_k^{-1} \bG_k(\htheta_k)$ in both algorithm (\ref{eq:Adaptation}) and
(\ref{eq:Enhancement}).
...
\bibitem{Sadegh1998} Sadegh, P. and Spall,
  ...
  approximation. \textit{IEEE Transactions on Automatic Control}, 44,
  231-237.
...
\bibitem{Farebrother1988}
Farebrother, R. W. (1988). \textit{Linear least squares computations}. Marcel Dekker, Inc..
