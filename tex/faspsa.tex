...
\begin{abstract}
Stochastic approximation applies in both the gradient-free optimization (Kiefer-Wolfowitz) and the gradient-based setting (Robbins-Monro). The idea of simultaneous perturbation (SP) is now well established. This paper discusses an efficient way of implementing both the adaptive SP algorithms and their enhancements (feedback and optimal weighting incorporated), using Woodbury matrix identity. Basically, instead of estimating the Hessian matrix directly, this paper deals with the estimation of the inverse of the Hessian matrix. Furthermore, the pre-conditioning steps, which are required in early iterations, are imposed on the Hessian inverse rather than the Hessian itself. Numerical results also demonstrate the superiority of this efficient implementation.
...
The essence of SP idea as applied in stochastic Newton's method is to approximately accurately and efficiently estimate the Hessian matrix of the loss function under the minimization setting, or to estimate the Jacobian matrix of the objective function under the root-finding setting. Throughout this paper, noisy function evaluations are available in 2SPSA algorithms, and noisy gradient evaluations in 2SG algorithms. 
 
Within this paper, all components in the perturbation sequences (both $ \bDelta_k $ and $ \tDelta_k $) are symmetric \textit{Bernoulli} distributed. This is completely valid and efficient as shown in \cite{Sadegh1998}. Using this particular perturbation (all components are either 1 or -1), following relationship holds:
\begin{equation} \label{eq:symmetry}
\bDelta_k=\bDelta_k^{-1}, \tDelta_k=\tDelta_k^{-1}
\end{equation}

This paper focuses on the updated estimate of the Hessian inverse itself, therefore all the pre-conditionings as appeared in \cite{Spall2000} and \cite{Spall2009} are in fact imposed on the inverse of the Hessian estimate. i.e., first consider the update of $ \oH_k^{-1} $, then perform the pre-conditioning on $ \oH_k^{-1} $ to obtain $ \ooH_k^{-1} $.

Additionally, every Hessian update and/or its inverse has to remain symmetric at every iteration. Bar-Itzhack \cite{Bar-Itzhack1998} shows that for any matrix $ \bm{P}\in\mathbb{R}^{p\times p} $, the closest symmetric matrix to $ \bm{P} $, in Frobenius (Euclidean) norm, is $ (\bm{P}+\bm{P}^T)/2 $. This further validates how \cite{Spall2000} and \cite{Spall2009} proceed the symmetrization.
...
An adaptive 2SPSA algorithm introduced in \cite{Spall2000} has the following two recursion:
...
An enhancement to adaptive 2SPSA algorithm in section \ref{2SPSA}, incorporated the feedback and weighting mechanism, as appeared in \cite{Spall2009} has the following two recursion:
...
\end{cases}
\end{equation}
where $\bG_k(\htheta_k)$, $\hH_k$, $\delta\bG_k$, and $\bG_k^{(1)}(\htheta_k\pm c_k\bDelta_k)$ are defined as in equation \ref{eq:notations}, the optimal weighting parameter is:
\begin{equation} \label{eq:weighting}
w_k=\frac{\tilde{c}_k^2c_k^2}{\sum_{i=0}^{k}\tilde{c}_i^2c_i^2}
\end{equation}

Use the the definition of equation \ref{eq:dy} and \ref{eq:HHat}. Below is the pre-symmetrized form of $ \hPsi_k $:
\begin{equation}
...
As in Spall \cite{Spall2009}, the Hessian updates always maintain their symmetry. The symmetry of previous update $ \oH_{k-1}$ guarantees that $\bDelta_k^{T}\oH_{k-1}\tDelta_k=\tDelta_k^{T}\oH_{k-1}\bDelta_k$. Denote
\begin{equation}
...
Now we have access to the noisy measurement of the gradient information, $\bG_k(\htheta_k)$, $\bG_k^{(1)}(\htheta_k+ c_k\bDelta_k)$ and $\bG_k^{(1)}(\htheta_k- c_k\bDelta_k)$ at each iteration $k$. An adaptive 2SG algorithm introduced in \cite{Spall2000} has the recursion as in equation \ref{eq:Adaptation} with
...
An enhancement to adaptive 2SG algorithm in section \ref{2SG}, incorporated the feedback and weighting mechanism, as appeared in \cite{Spall2009} has recursion form as equation \ref{eq:Enhancement} with optimal weighting defined in equation \ref{eq:weighting}. Now we have access to the noisy measurement of the gradient information, $\bG_k(\htheta_k)$, $\bG_k^{(1)}(\htheta_k+ c_k\bDelta_k)$ and $\bG_k^{(1)}(\htheta_k- c_k\bDelta_k)$ at each iteration $k$.
...
Note that we only use the symmetric Bernoulli perturbation sequence as suggested by \cite{Sadegh1998}, immediately $\bD_k$ is symmetric. As in \cite{Spall2009}, the HEssian updates always maintain symmetric. Therefore, $\hPhi_k$ is symmetric.

Eq. (3.12) in Spall \cite{Spall2009} provides a symmetrized feedback term $ \hPsi_k $, which can be written as:
\begin{equation} \label{eq:PsiHat}
\hPsi_k =(\hPhi_k+\hPhi_k^T)/2=\hPhi_k
\end{equation}


Denote
\begin{equation}
\bm{b}_k=w_k\left( \frac{1}{4c_k}(\delta\bG_k)-\oH_{k-1}\bDelta_k\right)
\end{equation}


Immediately, the new Hessian estimation is
\begin{align*}
\oH_k&=\oH_{k-1}+\bm{b}_k\bDelta_k^{-T}+\frac{w_k}{4c_k}\bDelta_k^{-1}(\delta\bG_k)^{T}
\end{align*}
...
\oH_k^{-1}
&=\bm{B}_k^{-1}-\bm{B}_k^{-1}\bDelta_k^{-1}\\
&~~~\cdot(\frac{4c_k}{w_k}+(\delta\bG_k){T}\bm{B}_k^{-1}\bDelta_k^{-1})^{-1}(\delta\bG_k)^{T}\bm{B}_k^{-1}
\end{dcases}
\end{equation}
where
...
Now we implement all four efficient implementation of stochastic Newton-like algorithm, described in section \ref{2SPSA}-\ref{Enhanced 2SG}, on the skew-quartic function in \cite{Spall2009}.
\begin{equation*}
...
\end{dcases}
\end{equation}
It can be verified that $L(\bm{\uptheta})$ is a strictly convex function, and the minimizer is $\bm{\uptheta}^{*}=\bm{0}$.

We follow the exact initialization and pre-conditioning steps in \cite{Spall2009} and the pracitical guidlines in \cite{Spall2000}. Below is the plot for these four implementations of the normalized loss functions $[L(\htheta_k)-L(\bm{\uptheta}^{*})]/[L(\htheta_0)-L(\bm{\uptheta}^{*})]$ against iteration $k$ averaging over 10 simulation runs.
...
Pushpendre Rastogi would like to thank the Department of Computer Science at the Johns Hopkins University during his graduate study. This work is supported in part by the Center of Imaging Science. Both the second and the third author received support from the Office of Naval Research (via Navy contract N00024-13-D6400).
